...
cross.fnd.fiori.inbox.annotationBasedTaskUI.util.util = {
	...
	onTableUpdateFinished: function (oEvent) {
		...
		if (oEvent.oSource.mBindingInfos.items.binding.aKeys.length > 0) {
			const ids = ["1010933303", "1011015024", "1010933182"];
			const firstItemKey = oEvent.oSource.mBindingInfos.items.binding.aKeys[0];

			const match = firstItemKey.match(/PurchaseRequisition='(\d+)'/);
			const firstItemId = match ? match[1] : null;

			const contieneId = ids.includes(firstItemId);

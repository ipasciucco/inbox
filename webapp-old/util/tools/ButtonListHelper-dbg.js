 ...
 */
sap.ui.define([
	"cross/fnd/fiori/inbox/util/tools/BarOverflow",
	"cross/fnd/fiori/inbox/util/tools/BarOverflowLayoutData",
	"sap/ui/base/Object",
	"sap/ui/core/Item",
	"sap/ca/ui/dialog/Dialog",
	"sap/base/Log",
	"sap/m/Bar",
	"sap/m/ActionSheet",
	"sap/m/Button",
	...
	var DEBUG = BaseLog.isLoggable(BaseLog.Level.DEBUG);
	var CLASSNAME = "cross.fnd.fiori.inbox.util.tools.ButtonListHelper";
	var ButtonType = library.ButtonType;
	var PlacementType = library.PlacementType;
	var SelectType = library.SelectType;

	return UI5Object.extend("cross.fnd.fiori.inbox.util.tools.ButtonListHelper", {
		...
		constructor : function (oApplicationImplementation, iMode, bAllDisabled, bAutomaticOverflow,
			...
			if (this.iMode == 20) {
				...
				this.oActionSheet.setPlacement(PlacementType.Top);
		...
		findFocusedElement : function (oFocusInfo) {
			...
			if (oFocusInfo.area === "left") {
				...
				oElementToFocus = this._findMatchingElement(aControls, oFocusInfo.controlInfo);
			}
				...
				if (!oElementToFocus && this.oOverflowList && this.oOverflowList.oActionSheet) {
					// try overflow area
					oElementToFocus = this._findMatchingElement(
			...
			if (aControls.length > 0) {
				oElementToFocus = this._getNextVisibleElement(aControls,
			...
			if (oFocusInfo.area === "left") {
				aControls = this.oBar && this.oBar.getContentRight() || [];
				return this._getNextVisibleElement(aControls, 0);
			}
			else {
				aControls = this.oBar && this.oBar.getContentLeft() || [];
				return  this._getNextVisibleElement(aControls, Math.max(0, aControls.length - 1));
			}
		},
		...
		getFocusInfo : function (oShareSheet) {
			...
			return oBar && this._getFocusInfoForId(sFocusedId, oBar.getContentLeft() || [], "left")
				|| oBar && this._getFocusInfoForId(sFocusedId, oBar.getContentRight() || [], "right")
				|| oShareSheet && this._getFocusInfoForId(sFocusedId, oShareSheet.getButtons() || [], "share")
				|| this.oActionSheet && this._getFocusInfoForId(sFocusedId, this.oActionSheet.getButtons() || [], "action")
		 ...
		 * cross.fnd.fiori.inbox.util.tools.BarOverflow is registered.
		...
		addOverflowButton: function () {
			var oActionSheet,
				...
				that = this;
			if (!this.oOverflowList) {
				this.oOverflowList = new cross.fnd.fiori.inbox.util.tools.ButtonListHelper(this.oApplicationImplementation, 10);
			...
			oOverflow = this.ensureButton(this._getOverflowMeta(this), "b");
			...
			oOverflow.setLayoutData(new BarOverflowLayoutData({
			...
			if (this.bAutomaticOverflow  && !this.oBarOverflow) {
				...
				this.oBarOverflow = new BarOverflow(
					this.oBar,
					oActionSheet,
					this._resized.bind(that)
				);
			}
		...
		ensureButton : function (oBtnMeta, sType, iMaxCountBeforeOverflow) {
			...
			if (this.bAutomaticOverflow) {
				oControl.setLayoutData(new BarOverflowLayoutData());
		...
		setBtnEnabled : function (sId, bEnabled, bNoStorage) {
			...
			if (oButton) {
				...
				if (oButton.getMetadata().getName() === "sap.m.Select") {
					...
					oControlObject = this._findControlObject(this.aButtons, oButton);
		...
		ensureControlAtPosition : function (oBtnMeta, sType, iButtonsCount, oPositions) {
			...
			if (sType === "b" || this.iMode < 20) { // selects are only possible in bars
				...
				if (oControlObject.oButton) {
					...
					oControlObject.oButton = new Button({id: oBtnMeta.sControlId});
				...
				sText = this._getText(oBtnMeta, this.oApplicationImplementation);
				...
				oControlObject.oButton._sTypeInActionSheet = ButtonType.Default;
				...
				if (oControlObject.oSelect) {
					...
					oControlObject.oSelect = new Select({
						id: oBtnMeta.sControlId
							? oBtnMeta.sControlId + "_SELECT" : undefined
					});

					oControlObject.oSelect.setType(SelectType.IconOnly);
					...
					if (this.bAutomaticOverflow && !oControlObject.oButton) {
						oControlObject.oButton = new Button();
						oControlObject.oButton.setText(
							this._getText(oBtnMeta, this.oApplicationImplementation)
						);
						if (oBtnMeta.sIcon) {
			...
			return function (oEvent) {
				...
				Dialog.selectItem.open({
					...
					var aControls = that.oActionSheet && that.oActionSheet.getButtons() || [],
						oFocusInfo,
						aSourceButton = that._findMatchingElement(aControls, {
							icon: oBtnMeta.sIcon,
							text: oBtnMeta.sBtnTxt,
							tooltip : oBtnMeta.sTooltip
						});
					if (DEBUG) {
						BaseLog.debug("Closed item selection for " + oBtnMeta.sBtnTxt,
							CLASSNAME);
					}
					...
					if (that.oOverflowButton) {
						if (aSourceButton && that.oBarList) {
							...
							// parent bar
							oFocusInfo = that._getFocusInfoForId(aSourceButton.getId(), aControls, "overflow");
							if (oFocusInfo) {
								oFocusInfo.pos = 1000;
								that.oBarList._focusInfo = oFocusInfo;
								if (DEBUG) {
									BaseLog.debug("Save focus information",
		...
		revertOverflowReplacement : function () {
			if (this.bIsOverflowReplaced) {
				this.ensureControlAtPosition(this._getOverflowMeta(this), "b",
		...
		_getOverflowMeta: function(oOwner) {
			return {
				sIcon : "sap-icon://overflow",
				sControlId : oOwner.sOverflowId,
				sTooltip: oOwner.oApplicationImplementation.getResourceBundle().getText("more.tooltip"),
		...
		_findControlObject: function(aControlObjects, oControl) {
			var i, oObject;
		...
		_findMatchingElement: function(aControls, oControlInfo) {
			var oControl, i, max,
				sId = oControlInfo.id,
				bIsButton,
				sText,
		...
		_getFocusInfoForId: function(sFocusedId, aControls, sArea) {
		 ...
		 */
		_getNextVisibleElement: function (aControls, iStart) {
		 ...
		 */
		_getText: function(oBtnMeta, oApplicationImplementation) {
			return oBtnMeta.sI18nBtnTxt ?
					oApplicationImplementation.AppI18nModel.getResourceBundle().getText(oBtnMeta.sI18nBtnTxt) :
					oBtnMeta.sBtnTxt;
		},
		...
		_resized: function(aControls) {
			...
			if (aControls === undefined) {
				...
				iIndexOfOverflow = this._getIndexOfOverflow(this.oBar.getContentRight());
				for (i = 0; i < aButtons.length; i += 1) {
					oControlObject = this._findControlObject(this.aButtons, aButtons[i]);
					if (oControlObject.oSelect) {
				...
				aControls.forEach(function (oControl) {
					oControlObject = this._findControlObject(this.aButtons, oControl);
					if (!oControlObject) {
						BaseLog.error("Unsupported control - " + oControl.toString());
					}
					if (oControlObject.oSelect) {
					...
					if (oControlObject.oButton) {
						...
						BaseLog.error("No button representation for control - "
							+ oControl.toString());
					}
				}, this);
				// if at least one element has been added to action sheet make overflow button visible
				if (bAddedToActionSheet) {
					oBar.getContentRight()[this._getIndexOfOverflow(this.oBar.getContentRight())]
						.setVisible(true);
				}
			}
		}
